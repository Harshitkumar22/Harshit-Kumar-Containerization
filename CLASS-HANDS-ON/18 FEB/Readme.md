...
```bash
docker network create Harshit_bridge
docker network ls
docker network inspect Harshit_bridge
```
...
```bash
docker run -dit --name container1 --network Harshit_bridge nginx

docker run -dit --name container2 --network Harshit_bridge busybox
```
